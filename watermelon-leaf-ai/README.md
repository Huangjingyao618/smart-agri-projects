...
> YOLOv8n-seg 实例分割 + OpenCV 面积计算 · 批量自动化测量西瓜叶片面积 · mAP50 = 0.9843

---

## 📋 项目简介

基于 YOLOv8n-seg 实例分割模型，自动识别西瓜叶片轮廓并计算叶面积。支持**单张测量**与**批量自动化处理**，输出 CSV 格式的面积报告。训练精度 mAP50 达到 **0.9843**。
...
- 实现精度极高的实例分割模型 (mAP50 = 0.9843)，远超课程要求
...
> *(运行截图占位 — 请替换为实际截图)*
>
> ![实例分割效果](./images/segmentation-result.png)
> *YOLOv8-seg 叶片轮廓分割效果*
>
> ![批量测量结果](./images/batch-result.png)
> *batch_measure.py 批量测量终端输出*
