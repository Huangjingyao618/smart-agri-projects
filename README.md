...
### 物联网 & 云
- **协议:** MQTT v3.1.1（CONNECT / SUBSCRIBE / PUBLISH / PINGREQ）
- **云平台:** 中国移动 OneNET IoT
...
本仓库项目为我在 **长江大学智慧农业专业** 课程学习期间完成。部分通信模块代码（ESP8266 MQTT、小程序 MQTT WebSocket）由课程教师提供或基于工具生成，其余代码由我个人开发。我在每个项目中承担的主要工作包括：

- **全链路架构实践** — 从硬件传感器 → 云平台 → 用户端小程序的完整数据通路设计与实现
- **嵌入式驱动开发** — STM32 外设驱动编写（DHT11、OLED、LED、蜂鸣器、按键等），集成 ESP8266 MQTT 通信模块
- **AI 模型训练与部署** — 数据集准备、模型选型、训练调参、推理部署
- **前后端全栈开发** — 微信小程序 UI 开发、MQTT 通信模块集成
